
# 1.) Ideation of Snow Show
Last Employed Signman clearing out road of heavy snowstorms (Major Stake) in turn of Directioning tourists to Yulegiving Festival (Impossible) (Goal) 

A Lone Signman, A Disaster Prevention Division Groups of men,  under the snowstorm calamity, must navigate through the tower at the top and light up the beacon that banishes the storm. 


### THEME
 SUSPENSE / 

~~The theme of useing the power of the people / culture / festival to clear the heavily damaging weather and prevent the collapsing world of white snow~~

The theme of man / robot gaining sentience and fulfills its purpose against all odds of heavy snowstorm to light up the beacon (causal effect) clears the storm


WHO
### Protagonist 

Signman  - Last Signmens
"Kind of sign' , Kind of Man" james baxter type design

Signboard has facial features

- Tsukumogami
- Snow bring it life
- Lighthouse Tower act as a failsafe towards the calamity

Lighthouse- man
- "Disaster Prevention Division" Government of people
- ~~Only one Signman left in the region~~ *canned*
- Ability to Direct / Misdirect misfortune / flow of attack


### Action
 - wake up (push pull)
 - Clear out the snow (lift idk)
 - Direct people out <- this ones questionable
 - or light up the light house ....(????)
 - Lighting up the bacon (PULL)

### Obstacle
 - Snowstorm caused by the gods
 - Bro is in calamity mode (red sky / hue)
 - Tower Interior is damaged by the storm
 - 
### Stake
- Snow Festival or whatever helps regulate the weather of snow
- Without people to direct, snowstorm will keep piling on
- End of the Festival means end of the world
- Idk any other steak

### Resolve
- Lighthouse is lit — providing visible light and heat to the area.
- the snow storm is subsided.
- ~~the "Snow" in question is a moth-like creature providing freezing essence~~
- 

### INITIAL STORYLINE
#### Timeline Board

Scene starts in interior
0 - 10 
Bro woke up
- may not be his own bed in a room in a house
- recent storm made signman collapse in a tower idk
- does the lighthouse functions the same way????

11 - 30 
uses the spin attack to clear obstacle on your way to work
(Push / Pull) demonstration to clear out props caused by storm

30 - 40 
Scene trans - from interior to outside - Red Sky with heavy snowstorm (?)

40 - 60 
Clear out the snow
## Refs

### Reasonables
### Renders
mayro odyssey sand kingdom
- expectiation - dessert = heat
- reality - dessert with cold weather 
![[image.webp|348x183]]


- mood board
- red hue suggests a calamity of some kind
![[image-1.webp|290x193]]

### Movie Referes
- Tom Cruise Mission Impossible 

### Ice Age
- this one is ice age bcz its neatiness
![[image-2.webp|292x170]]

### The Legend of Zelda : Majora Mask
- Snowhead temple
- major inspiration 
- Snowhead temple in hail and blizzard weather, clearing the temple clears out the foggy weatehr
![[image-3.webp|281x169]]
### Render Style
idk maybe ice age

### Dog Man Red Hue Render
![[image-5.webp|318x179]]


#### Character References
#### Prop References

- Sign???
- Driller

#### Lighting References
Scene take palce in waning sun - sunset
- before turning dark
- lighting hard panned on red hue
- 
#### Render References

### Initial Sketches



# 2.) Ideation of idk 








